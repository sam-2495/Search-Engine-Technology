...
public class SnowballProgram {
	...
	/*
	 * extern void debug(struct SN_env * z, int number, int line_count) { int i; int
	 * limit = SIZE(z->p); //if (number >= 0) printf("%3d (line %4d): '", number,
	 * line_count); if (number >= 0) printf("%3d (line %4d): [%d]'", number,
	 ...
	 * ch = z->p[i]; if (ch == 0) ch = '#'; printf("%c", ch); } } printf("'\n"); }
	 */
