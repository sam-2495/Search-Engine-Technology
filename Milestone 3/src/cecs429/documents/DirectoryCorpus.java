...
public class DirectoryCorpus implements DocumentCorpus {
     ...
     * RegisterFileDocumentFactory method. Otherwise, the corpus will not know what
     * to do with the files it finds. The LoadTextDirectory facade method can
     * simplify this initialization.
     *
     * @see
     */
    ...
    /**
     * Constructs a corpus over an absolute directory path, only loading files whose
     * file names satisfy the given predicate filter.
     */
    ...
    private HashMap<Integer, Document> readDocuments() throws IOException {
        ...
        Collections.sort(files, new Comparator<Path>() {
            @Override
            public int compare(Path u1, Path u2) {
                return u1.getFileName().toString().compareTo(u2.getFileName().toString());
            }
        });
    ...
    /**
     * Finds all file names that match the corpus filter predicate and have a known
     * file extension.
     */
    ...
    /**
     * Registers a factory method for loading documents of the given file extension.
     * By default, a corpus does not know how to load any files -- this method must
     * be called prior to getDocuments().
     */
	...
	/*
	 * public static DirectoryCorpus loadXMLDirectory(Path absolutePath, String
	 * fileExtension) { DirectoryCorpus corpus = new DirectoryCorpus(absolutePath);
	 * corpus.registerFileDocumentFactory(fileExtension,
	 * XmlFileDocument::loadXMLFileDocument); return corpus; }
	 */
}
